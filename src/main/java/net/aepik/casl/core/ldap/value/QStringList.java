/*
 * QStringList.java		0.1		23/05/2006
 * 
...
/**
 * Une liste de QString.
**/

public class QStringList implements SchemaValue {
...
////////////////////////////////
	...
	**/
	public String[] getValues() {
		...
		Iterator<QString> it = liste.iterator();
	...
	**/
	public boolean setValue( String value ) {
		...
		if( ( firstBracket = value.indexOf( 40 ) )<0 ) {
			if( ok = QString.isValidFormat( value ) ) {
				newListe.add( new QString( value ) ) ;
				ok = true ;
			}
			...
			try {
				do {
					...
					// Tout est ok, on récupère la sous-chaîne est on la teste.
						...
						if( ok = QString.isValidFormat( tmp ) )
							newListe.add( new QString( tmp ) );
					}
	...
	**/
	public boolean setValues( String[] values ) {
		...
		for( int i=0; i<values.length && ok; i++ ) {
			if( ok = QString.isValidFormat( values[i] ) )
				newListe.add( new QString( values[i] ) );
		}
